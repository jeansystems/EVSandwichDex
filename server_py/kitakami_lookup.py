...
def get_species_urls(POKEDEX):
    ...
    result = resp.json()
    species_urls = [entry["pokemon_species"]["url"] for entry in result["pokemon_entries"]][0:10]
    pokedex_nums = [entry["entry_number"] for entry in result["pokemon_entries"]][0:10]
